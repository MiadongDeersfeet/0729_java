...
public class C_Switch {
	...
	public void method0() {
		...
		break; //브레이크를 만나면 밑으로 안내려가고 닫는 스코프 밖으로 나갑니다. 보통 case마다 break가 달려있다.
		 ...
		 * case "값" 정수, 문자, 실수, 문자열 : 실행할 코드; 
		 * 
		 * }
		 * 
		 * switch(정수 || 실수 || 문자 || 문자열) {
		 * case "(값1)정수 || 실수 || 문자 || 문자열" : 실행할 코드a; // 동등비교대상 == 값1 true일 경우 실행할 코드 a를 수행
		 * 
		 * case 값2 : 실행할 코드b; // 동등비교대상 == 값2 true일 경우 실행할 코드 b를 수행
		 * 
		 * case 값n : 실행할 코드n; // 동등비교대상 == 값n true일 경우 실행할 코드 n을 수행
		 * 
		 * 
		 * default : 실행할코드; // == else
		 * 
	...
	public void method1() {
		...
		*/
		...
		default : System.out.println("없는 메뉴입니다.");
		return; // 현재 실행되고 있는 메소드 영역을 아예 빠져나갑니다.
		        // 메소드를 호출한 곳으로 돌아간다는 의미.
		} // break 출구
		
		//if(price != 0) {
			System.out.println(price + "원 입니다.");
		} // return 출구0.0 => 메인 메소드로 이동
	
	public void login() {
		
		// 로그인 구현하기!
		// 기획 및 설계 단계(프로젝트 만들 때)
		
		// 기획 -> 뭘 만들지?
		// 설계 -> 어떻게 만들지?(개발자에게는 설계가 중요해요.)
		
		// 로그인 사용 -> 분석 -> 이해 하는 과정이 필요함 .
		// 로그인이란 뭘까...?
		...
		// 로그인 구현할 때 필요한 값 :
		// 1. 사용자가 입력한 아이디 값 -> (자료형을 생각해야함) 문자열
		...
		// 3. 가입 시 입력했던 아이디와 비밀번호가 하나라도 일치하지 않다면 로그인 실패 화면 출력
		...
		// 사용자가 로그인 시 입력한 비밀번호값과 회원가입시 가입한 비밀번호값이 동일하다면
